import type { OptionType } from '@/config/dictions';
import { getProSorter } from '@/utils/tools';
import protobuf from 'protobufjs';
import { request } from 'umi';
import type { BaseResponse } from './entities';
 ...
 * 获取protobuf类型对象
 */
let proto: Record<string, protobuf.Type>
...
export async function getProto() {
    ...
    return protobuf.load("/protobuf/instruct.proto")
        .then(root => {
            ...
            const WarnNote = root.lookupType("instruct.WarnNote")
            const ServerStatus = root.lookupType("instruct.ServerStatus")
            const QuakeUpdate = root.lookupType("instruct.QuakeUpdate")
            return proto = {
                Authentication,
                Consequence,
                WsHeartbeat,
...
export { post, get, put, del, deletes, callProTableData, callOptionsData, arrayBufferToJSON };
