...
export function ProFormModal<T>(props: ProTableProps<T, AnyForm> & { hasRestFooter?: boolean, refreshTime?: { flag: boolean, name: string } } & {
    ...
    return <Modal visible={modal.visible} title={modal.title} className={styles.modal}
        ...
        {...hasRestFooter && {
            footer: [
                <Button onClick={() => form.current?.setFieldsValue({ ...modal.defaultFormValues, content: '', result: '' })}>重置</Button>,
                <Button type="primary" onClick={() => form.current?.validateFields().then(val => {
                    setloading(true)
                    // onSubmit必须返回Promise
                    return onSubmit?.(val)
